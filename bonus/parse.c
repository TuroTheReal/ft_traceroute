	...
	for (int i = 1; i < argc; i++) {
		if (argv[i][0] == '-' && argv[i][1] == '-') {
			...
			if (argv[i][2] != '\0' && argv[i][1] != '-' &&
				...
				|| (argv[i][1] == 'p') || (argv[i][1] == 'm')
				|| (argv[i][1] == 'n') || (argv[i][1] == 'i')
				|| (argv[i][1] == 'q') || (argv[i][1] == 'w'))) {
...
void parse_args(int argc, char** argv, t_trace *trace) {
	...
	while ((opt = getopt_long(argc, argv, "V?p:m:q:i:w:hn", long_options, NULL)) != -1) {
		switch (opt) {
			case 'n': trace->no_dns = 1; break;
			case 'V':
				print_version();
				exit(0);
			...
			case '?':
				// D'abord vérifier si c'était une demande d'aide explicite
				if (argv[optind - 1] && strcmp(argv[optind - 1], "-?") == 0) {
					print_help();
					exit(0);
				}

				// Sinon, c'est une option invalide
				if (optopt) {
